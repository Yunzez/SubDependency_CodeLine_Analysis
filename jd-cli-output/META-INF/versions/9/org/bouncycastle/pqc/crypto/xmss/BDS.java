...
public final class BDS implements Serializable {
  ...
  BDS(org.bouncycastle.pqc.crypto.xmss.BDS paramBDS) {
    ...
    this.treeHashInstances = new ArrayList<>();
    for (Iterator<BDSTreeHash> iterator = paramBDS.treeHashInstances.iterator(); iterator.hasNext();)
  ...
  private void initialize(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2, OTSHashAddress paramOTSHashAddress) {
    ...
    for (byte b = 0; b < 1 << this.treeHeight; b++) {
      ...
      hashTreeAddress = (HashTreeAddress)(new HashTreeAddress.Builder()).withLayerAddress(hashTreeAddress.getLayerAddress()).withTreeAddress(hashTreeAddress.getTreeAddress()).withTreeIndex(b).withKeyAndMask(hashTreeAddress.getKeyAndMask()).build();
      while (!this.stack.isEmpty() && ((XMSSNode)this.stack.peek()).getHeight() == xMSSNode.getHeight()) {
        ...
        if (i >= 3 && (i & 0x1) == 1 && xMSSNode.getHeight() >= this.treeHeight - this.k && xMSSNode
