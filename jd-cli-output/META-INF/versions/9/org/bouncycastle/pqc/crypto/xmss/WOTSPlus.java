...
final class WOTSPlus {
  ...
  WOTSPlusSignature sign(byte[] paramArrayOfbyte, OTSHashAddress paramOTSHashAddress) {
    ...
    List<Integer> list2 = convertToBaseW(XMSSUtil.toBytesBigEndian(i, j), this.params
        .getWinternitzParameter(), this.params.getLen2());
    list1.addAll(list2);
  ...
  WOTSPlusPublicKeyParameters getPublicKeyFromSignature(byte[] paramArrayOfbyte, WOTSPlusSignature paramWOTSPlusSignature, OTSHashAddress paramOTSHashAddress) {
    ...
    for (byte b = 0; b < this.params.getLen(); b++) {
      paramOTSHashAddress = (OTSHashAddress)(new OTSHashAddress.Builder()).withLayerAddress(paramOTSHashAddress.getLayerAddress()).withTreeAddress(paramOTSHashAddress.getTreeAddress()).withOTSAddress(paramOTSHashAddress.getOTSAddress()).withChainAddress(b).withHashAddress(paramOTSHashAddress.getHashAddress()).withKeyAndMask(paramOTSHashAddress.getKeyAndMask()).build();
      arrayOfByte[b] = chain(paramWOTSPlusSignature.toByteArray()[b], ((Integer)list1.get(b)).intValue(), this.params
          .getWinternitzParameter() - 1 - ((Integer)list1.get(b)).intValue(), paramOTSHashAddress);
    } 
  ...
  private byte[] chain(byte[] paramArrayOfbyte, int paramInt1, int paramInt2, OTSHashAddress paramOTSHashAddress) {
    ...
    byte[] arrayOfByte1 = chain(paramArrayOfbyte, paramInt1, paramInt2 - 1, paramOTSHashAddress);
    ...
    byte[] arrayOfByte3 = this.khf.PRF(this.publicSeed, paramOTSHashAddress.toByteArray());
    byte[] arrayOfByte4 = new byte[i];
    for (byte b = 0; b < i; b++)
      arrayOfByte4[b] = (byte)(arrayOfByte1[b] ^ arrayOfByte3[b]); 
    arrayOfByte1 = this.khf.F(arrayOfByte2, arrayOfByte4);
    return arrayOfByte1;
  }
