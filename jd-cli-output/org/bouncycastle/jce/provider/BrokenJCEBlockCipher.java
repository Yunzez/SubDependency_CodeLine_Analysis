...
public class BrokenJCEBlockCipher implements BrokenPBE {
  ...
  protected void engineSetMode(String paramString) {
    ...
    if (str.equals("ECB")) {
      ...
      if (str.length() != 3) {
        ...
        this.cipher = new PaddedBufferedBlockCipher(new OFBBlockCipher(this.cipher.getUnderlyingCipher(), i));
      } else {
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (paramKey instanceof BCPBEKey) {
      cipherParameters = BrokenPBE.Util.makePBEParameters((BCPBEKey)paramKey, paramAlgorithmParameterSpec, this.pbeType, this.pbeHash, this.cipher.getUnderlyingCipher().getAlgorithmName(), this.pbeKeySize, this.pbeIvSize);
      if (this.pbeIvSize != 0)
      ...
      if (this.ivLength != 0) {
        cipherParameters = new ParametersWithIV(new KeyParameter(paramKey.getEncoded()), ((IvParameterSpec)paramAlgorithmParameterSpec).getIV());
        this.ivParam = (ParametersWithIV)cipherParameters;
      } else {
        cipherParameters = new KeyParameter(paramKey.getEncoded());
      } 
      ...
      cipherParameters = new RC2Parameters(paramKey.getEncoded(), ((RC2ParameterSpec)paramAlgorithmParameterSpec).getEffectiveKeyBits());
      if (rC2ParameterSpec.getIV() != null && this.ivLength != 0) {
        ParametersWithIV parametersWithIV = new ParametersWithIV(cipherParameters, rC2ParameterSpec.getIV());
      ...
      cipherParameters = new RC5Parameters(paramKey.getEncoded(), ((RC5ParameterSpec)paramAlgorithmParameterSpec).getRounds());
    ...
    if (this.ivLength != 0 && !(cipherParameters instanceof ParametersWithIV)) {
      ...
      if (paramInt == 1 || paramInt == 3) {
        ...
        cipherParameters = new ParametersWithIV(cipherParameters, arrayOfByte);
        this.ivParam = (ParametersWithIV)cipherParameters;
      } else {
