...
public class BCDSTU4145PublicKey implements ECPublicKey, ECPublicKey, ECPointEncoder {
  ...
  private void populateFromPubKeyInfo(SubjectPublicKeyInfo paramSubjectPublicKeyInfo) {
    ...
    if (this.dstuParams != null) {
      ...
      if (this.dstuParams.isNamedCurve()) {
        ...
        this.ecSpec = new ECNamedCurveSpec(str, ellipticCurve, eCPoint, eCParameterSpec.getN(), eCParameterSpec.getH());
      } else {
  ...
  public byte[] getEncoded() {
    ...
    try {
      subjectPublicKeyInfo = new SubjectPublicKeyInfo(new AlgorithmIdentifier(UAObjectIdentifiers.dstu4145be, x962Parameters), new DEROctetString(arrayOfByte));
    } catch (IOException iOException) {
      return null;
    } 
