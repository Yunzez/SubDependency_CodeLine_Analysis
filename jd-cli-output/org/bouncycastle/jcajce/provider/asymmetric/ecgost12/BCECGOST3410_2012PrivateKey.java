...
public class BCECGOST3410_2012PrivateKey implements ECPrivateKey, ECPrivateKey, PKCS12BagAttributeCarrier, ECPointEncoder {
    ...
    if (aSN1Primitive instanceof ASN1Sequence && (ASN1Sequence.getInstance(aSN1Primitive).size() == 2 || ASN1Sequence.getInstance(aSN1Primitive).size() == 3)) {
      ...
      if (x962Parameters.isNamedCurve()) {
        ...
        if (x9ECParameters == null) {
          ...
          this.ecSpec = new ECNamedCurveSpec(ECGOST3410NamedCurves.getName(aSN1ObjectIdentifier), ellipticCurve, EC5Util.convertPoint(x9ECParameters1.getG()), x9ECParameters1.getN(), x9ECParameters1.getH());
        } else {
  ...
  public byte[] getEncoded() {
    ...
    if (this.gostParams != null) {
      ...
      try {
        PrivateKeyInfo privateKeyInfo = new PrivateKeyInfo(new AlgorithmIdentifier(aSN1ObjectIdentifier, this.gostParams), new DEROctetString(arrayOfByte));
    ...
    try {
      PrivateKeyInfo privateKeyInfo = new PrivateKeyInfo(new AlgorithmIdentifier(aSN1ObjectIdentifier, x962Parameters.toASN1Primitive()), eCPrivateKey.toASN1Primitive());
      return privateKeyInfo.getEncoded("DER");
    } catch (IOException iOException) {
      return null;
    } 
