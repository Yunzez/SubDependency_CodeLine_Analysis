...
public class BCECPublicKey implements ECPublicKey, ECPublicKey, ECPointEncoder {
  ...
  public byte[] getEncoded() {
    boolean bool = (this.withCompression || Properties.isOverrideSet("org.bouncycastle.ec.enable_pc")) ? true : false;
    AlgorithmIdentifier algorithmIdentifier = new AlgorithmIdentifier(X9ObjectIdentifiers.id_ecPublicKey, ECUtils.getDomainParametersFromName(this.ecSpec, bool));
    byte[] arrayOfByte = this.ecPublicKey.getQ().getEncoded(bool);
    return KeyUtil.getEncodedSubjectPublicKeyInfo(algorithmIdentifier, arrayOfByte);
  }
