...
public class GCMBlockCipher implements AEADBlockCipher {
  ...
  public GCMBlockCipher(BlockCipher paramBlockCipher, GCMMultiplier paramGCMMultiplier) {
    ...
    if (paramGCMMultiplier == null)
      paramGCMMultiplier = new Tables4kGCMMultiplier(); 
  ...
  public int processByte(byte paramByte, byte[] paramArrayOfbyte, int paramInt) throws DataLengthException {
    ...
    if (++this.bufOff == this.bufBlock.length) {
      processBlock(this.bufBlock, 0, paramArrayOfbyte, paramInt);
      if (this.forEncryption) {
        this.bufOff = 0;
      } else {
  ...
  public int processBytes(byte[] paramArrayOfbyte1, int paramInt1, int paramInt2, byte[] paramArrayOfbyte2, int paramInt3) throws DataLengthException {
    ...
    if (this.forEncryption) {
      if (this.bufOff != 0)
        while (paramInt2 > 0) {
          paramInt2--;
          this.bufBlock[this.bufOff] = paramArrayOfbyte1[paramInt1++];
          if (++this.bufOff == 16) {
            processBlock(this.bufBlock, 0, paramArrayOfbyte2, paramInt3);
            this.bufOff = 0;
            b += true;
            break;
          } 
      ...
      while (paramInt2 >= 16) {
        processBlock(paramArrayOfbyte1, paramInt1, paramArrayOfbyte2, paramInt3 + b);
        paramInt1 += 16;
        paramInt2 -= 16;
        b += 16;
      } 
      if (paramInt2 > 0) {
        System.arraycopy(paramArrayOfbyte1, paramInt1, this.bufBlock, 0, paramInt2);
        this.bufOff = paramInt2;
      } 
    } else {
      for (byte b1 = 0; b1 < paramInt2; b1++) {
        this.bufBlock[this.bufOff] = paramArrayOfbyte1[paramInt1 + b1];
        if (++this.bufOff == this.bufBlock.length) {
          processBlock(this.bufBlock, 0, paramArrayOfbyte2, paramInt3 + b);
          System.arraycopy(this.bufBlock, 16, this.bufBlock, 0, this.macSize);
          this.bufOff = this.macSize;
          b += 16;
        } 
