...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        byte[] output = new byte[32]; // Here 32 is just a sample size. Ideally, you should calculate or handle the size dynamically.

        // Encryption with Apache Commons Crypto
        encipher.init(Cipher.ENCRYPT_MODE, keySpec, new javax.crypto.spec.IvParameterSpec(initVector));
        int updateBytes = encipher.update(text.getBytes(), 0, text.getBytes().length, output, 0);
        int finalBytes = encipher.doFinal(text.getBytes(), 0, 0, output, updateBytes);
        byte[] finalEncrypted = new byte[updateBytes + finalBytes];
        System.arraycopy(output, 0, finalEncrypted, 0, updateBytes + finalBytes);
        String encryptedBase64Commons = Base64.getEncoder().encodeToString(finalEncrypted);
        System.out.println("Apache Commons Crypto Encrypted: " + encryptedBase64Commons);

        // Decryption with Apache Commons Crypto
        CryptoCipher decipher = Utils.getCipherInstance("AES/CBC/PKCS5Padding", properties);
        decipher.init(Cipher.DECRYPT_MODE, keySpec, new javax.crypto.spec.IvParameterSpec(initVector));  // Initialization of the decipher
        byte[] decryptedOutput = new byte[32];
        updateBytes = decipher.update(Base64.getDecoder().decode(encryptedBase64Commons), 0, Base64.getDecoder().decode(encryptedBase64Commons).length, decryptedOutput, 0);
        finalBytes = decipher.doFinal(Base64.getDecoder().decode(encryptedBase64Commons), 0, 0, decryptedOutput, updateBytes);
