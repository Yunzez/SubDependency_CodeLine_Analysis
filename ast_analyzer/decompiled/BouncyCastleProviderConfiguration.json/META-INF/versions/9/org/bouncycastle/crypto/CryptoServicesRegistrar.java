...
public final class CryptoServicesRegistrar {
  ...
  private static Object[] lookupProperty(Property paramProperty) {
    ...
    if (map == null || !map.containsKey(Property.access$000(paramProperty))) {
      arrayOfObject = globalProperties.get(Property.access$000(paramProperty));
    } else {
  ...
  private static <T> void localSetGlobalProperty(Property paramProperty, T... paramVarArgs) {
    ...
    globalProperties.put(Property.access$000(paramProperty), (Object[])paramVarArgs);
  }
  
  public static <T> T[] clearGlobalProperty(Property paramProperty) {
    checkPermission(CanSetDefaultProperty);
    localClearThreadProperty(paramProperty);
    return (T[])globalProperties.remove(Property.access$000(paramProperty));
  }
  ...
  private static DHParameters toDH(DSAParameters paramDSAParameters) {
    ...
    return new DHParameters(paramDSAParameters.getP(), paramDSAParameters.getG(), paramDSAParameters.getQ(), j, 0, null, new DHValidationParameters(paramDSAParameters
          .getValidationParameters().getSeed(), paramDSAParameters.getValidationParameters().getCounter()));
  }
